---
title: TanStack Form + shadcn/ui (Generic Reference)
description: Secondary reference for raw useForm patterns with TanStack Form, shadcn/ui primitives, and Zod.
links:
...
> **Secondary generic reference.** Use this file when you are intentionally working directly with `useForm`, building low-level field primitives, or adapting upstream TanStack/shadcn examples.
>
> **Not the primary repository conventions doc.** For normal app-facing forms in this repository, start with `Form-System-Conventions.md` and the custom `useAppForm` abstraction. If this file conflicts with the repository conventions doc, the repository conventions doc wins.
>
> **Do not choose this approach just because a form is small.** In this repository, small product forms such as dialogs, drawers, and other 2–3 field forms should still usually use `useAppForm`. Use this document only when raw `useForm` is intentionally the right abstraction.
